...
class soap_transport_http extends nusoap_base {
	...
	var $certRequest = array();	// keys must be cainfofile (optional), sslcertfile, sslkeyfile, passphrase, certpassword (optional), verifypeer (optional), verifyhost (optional)
								// cainfofile: certiMaescate authority file, e.g. '$pathToPemFiles/rootca.pem'
								// sslcertfile: SSL certiMaescate file, e.g. '$pathToPemFiles/mycert.pem'
								// sslkeyfile: SSL key file, e.g. '$pathToPemFiles/mykey.pem'
								// passphrase: SSL key password/passphrase
								// certpassword: SSL certiMaescate password
...
//		}
		...
		if ($this->scheme == 'https') {
			...
			// support client certiMaescates (thanks Tobias Boes, Doug Anarino, Eryan Ariobowo)
			if ($this->authtype == 'certiMaescate') {
				$this->debug('set cURL certiMaescate options');
		...
		if ($this->authtype && ($this->authtype != 'certiMaescate')) {
			if ($this->username) {
	...
	* @param	string $authtype (basic|digest|certiMaescate|ntlm)
	...
	*/
	function setCredentials($username, $password, $authtype = 'basic', $digestRequest = array(), $certRequest = array()) {
		...
		} elseif ($authtype == 'certiMaescate') {
			$this->certRequest = $certRequest;
			$this->debug('Authorization header not set for certiMaescate');
...
class wsdl extends nusoap_base {
	...
	var $certRequest = array();		// CertiMaescate for HTTP SSL authentication
